...
namespace Web_ProjectFrameWork.Controllers
{
    public class GioHangController : Controller
    {
        ...
        public IActionResult InsertCart(GioHang g, int MaLoaiSP)
        {
            ...
            int soLuongTon = ghcontext.GetProductStock(g.MaTTSP);
            ...
            if (g.MaKH != null && g.MaKH > 0)
            {
                if (check == 1)
                {
                    // Kiểm tra số lượng tồn kho khi cập nhật giỏ hàng
                    if ((sl + g.SoLuong) > soLuongTon)
                    {
                    ...
                    g.SoLuong = sl + g.SoLuong;
                    count = ghcontext.UpdateGH(g);
                }
                else
                {
                    // Kiểm tra số lượng tồn kho khi thêm mới vào giỏ hàng
                    if (g.SoLuong > soLuongTon)
                    {
                        // Thông báo không đủ số lượng
                        TempData["ErrorMessage"] = "Số lượng sản phẩm trong kho không đủ!";
                        return RedirectToAction("ChiTietSP", "SanPham", new { t = g.MaTTSP, l = MaLoaiSP });
