...
CREATE TABLE `dssanpham_mua` (
  `MaSPM` int(10) UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TRIGGER `After_Insert_CTHD` AFTER INSERT ON `cthd` FOR EACH ROW BEGIN
    ...
    INSERT INTO dssanpham_mua (MaHD, MaTTSP, SoLuong, ThanhTien)
    VALUES (NEW.MaHD, NEW.MaTTSP, NEW.SoLuong, NEW.ThanhTien);
...
CREATE TRIGGER `After_Update_CTHD` AFTER UPDATE ON `cthd` FOR EACH ROW BEGIN
    ...
    SET SoLuong = NEW.SoLuong, ThanhTien = NEW.ThanhTien 
    WHERE MaHD = NEW.MaHD AND MaTTSP = NEW.MaTTSP;
